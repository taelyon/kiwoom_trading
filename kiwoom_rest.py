...
class KiwoomRestClient:
    ...
    async def get_stock_daily_chart(self, code: str, base_dt: str = '', cont_yn: str = 'N', next_key: str = '') -> Dict:
        """주식 일봉 차트 데이터 조회 (ka10081) (비동기)"""
        try:
            await self._ensure_client()
            if not await self.check_token_validity():
                return {}
            ...
            # API 요청 제한 확인 및 대기
            await ApiLimitManager.check_api_limit_and_wait_async("일봉 차트 조회", request_type="minute_chart")
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if response.status_code == 429:
                        if attempt < max_retries:
                            self.logger.warning(f"일봉 차트 조회 429 에러(한도 초과). 3.5초 대기 후 재시도 ({attempt+1}/{max_retries})... {code}")
                            await asyncio.sleep(3.5)
                            continue
                        else:
                            self.logger.error(f"일봉 차트 조회 429 에러 최대 재시도 초과: {code}")
                            break
                    break
                except httpx.RequestError as timeout_err:
                    if attempt < max_retries:
                        self.logger.warning(f"일봉 차트 조회 타임아웃/네트워크 오류 ({attempt+1}/{max_retries}), 재시도 중... {code}")
                        await asyncio.sleep(1)
                        if attempt == max_retries - 1:
                            await self._reset_client()
                    else:
            ...
            if response.status_code == 200:
                response_data = response.json()
                self.logger.debug(f"[ka10081] 일봉 데이터 조회 성공: {code}, return_code={response_data.get('return_code')}")
                return response_data
            else:
                self.logger.error(f"일봉 차트 데이터 조회 실패: {response.status_code} / {code}")
                return {}

        except httpx.ReadTimeout:
            self.logger.warning(f"일봉 차트 데이터 조회 타임아웃 (60초 초과): {code}")
            return {}
        except httpx.ConnectTimeout:
            self.logger.warning(f"일봉 차트 데이터 조회 연결 타임아웃: {code}")
            return {}
        except Exception as e:
