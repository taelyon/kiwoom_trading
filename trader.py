...
class KiwoomTrader:
    ...
    def check_circuit_breaker(self, daily_profit_rate: float) -> bool:
        """당일 계좌 서킷브레이커 (-5.0%) 검사 및 상태 업데이트"""
        if daily_profit_rate <= self.circuit_breaker_threshold:
            if not self.circuit_breaker_active:
                self.circuit_breaker_active = True
                warn_msg = f"[RISK_WARN] 🚨 [서킷브레이커 발동] 당일 계좌 손실률({daily_profit_rate:.2f}%)이 기준({self.circuit_breaker_threshold}%) 이하로 떨어져 당일 신규 매수를 전면 차단합니다."
